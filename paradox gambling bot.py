...
def run_flask():
    port = int(os.environ.get('PORT', 8080))  # Use Replit's assigned port or default to 8080
    try:
...
WIN_MESSAGES = [
    "You're on fire! 🔥",
    "Lucky spin! 🍀",
    "The Paradox Casino is feeling generous today! 💸",
    "Winner winner chicken dinner! 🍗",
    "Cha-ching! 💰",
    "The odds are in your favor! ✨",
    "Jackpot energy! ⚡",
    "Your luck is through the roof! 🚀"
]

LOSS_MESSAGES = [
    "Better luck next time! 🎲",
    "So close, yet so far! 📏",
    "The Paradox machines are being stubborn today! 😤",
    "Don't give up! Your big win is coming! ✨",
    "That's how they get you! 🎯",
    "The house always wins... or does it? 🤔",
    "Time to break the losing paradox! 🔄",
...
async def bet(ctx, amount: int, lines: int = 1):
    ...
    if amount >= 100:
        message = await ctx.send("🎰 The Paradox Machine is spinning...")
        await asyncio.sleep(1.0)
        await message.edit(content="🎰 The Paradox Machine is spinning... 🍒")
    ...
    earned_achievements = check_achievements(
        user, amount, winnings, jackpot, user_balances[user], user_streaks[user]["wins"]
    )

    if winnings > 0:
        color = 0xf1c40f
        result_title = "🎰 Paradox Winner! 🎰"
        result_message = f"{random.choice(WIN_MESSAGES)}\nYou bet ${total_bet} and won ${winnings}!"
    else:
        color = 0x95a5a6
        result_title = "🎰 Paradox Says No! 🎰"
        result_message = f"{random.choice(LOSS_MESSAGES)}\nYou bet ${total_bet} and lost."
