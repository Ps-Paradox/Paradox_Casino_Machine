...
def init_db():
    """Initialize the SQLite database with comprehensive tables for users, games, and items."""
    conn = sqlite3.connect('casino.db')
    c = conn.cursor()
    
    # Users table for storing player data
...
def add_xp(user_id, amount):
    """Add XP to a user, handle level-ups, and award achievements."""
    user_data = get_user_data(user_id)
    ...
    update_user_data(user_id, {
        ...
        'achievements': json.dumps(json.loads(user_data['achievements']) + earned_achievements)
    })
...
def check_achievements(user_id, bet_amount, win_amount, jackpot_win, balance, streaks, game_type=None):
    """Check and award achievements based on user activity and game outcomes."""
    user_data = get_user_data(user_id)
...
@bot.event
async def on_ready():
    """Handle bot startup, set presence, and log readiness."""
...
async def bet(ctx, amount: int, lines: int = 1):
    """Spin the slot machine with an animated reveal."""
    ...
    if level_achievements:
        await ctx.send(f"🏆 {ctx.author.mention} earned: " + ", ".join(f"**{ACHIEVEMENTS[a]['name']}**" for a in level_achievements))
    if drop_message:
